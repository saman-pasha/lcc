...
(defun specify-type-value< (desc ir)
  (let ((l (cdr (last desc)))
    ...
    (cond ((and (listp l) (> (length desc) 2) (key-eq (nth (- (length desc) 2) desc) 'FUNCTION)) ; #' function initializer
          ...
          ((and (listp l) (> (length desc) 2) (key-eq (nth (- (length desc) 2) desc) 'QUOTE)) ; ' list initializer
           (let ((quoted (cadr (nthcdr (- (length desc) 2) desc))))
             (if (key-eq (car quoted) '|lambda|)
                 (progn ; lambda
                   (display "QQLL" (listp l) (length desc) (car desc) (cadr desc) (nthcdr (- (length desc) 2) desc))
                   ;; todo: pointer to function specifier 
                   )
                 (progn ; list
