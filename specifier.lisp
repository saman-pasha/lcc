...
(defun specify-type< (desc ir &optional noVar)
  (let ((len (if (listp desc) (length desc) 1))
	      ...
	      ((= len 3) (if (key-eq (nth 0 desc) '|const|)
		                 ...
		                 (if (find (nth 1 desc) *modifiers* :test #'key-eq)
			                 ...
			                 (progn
			                   ...
			                   (setq variable (nth 1 desc))
			                   (setq array (nth 2 desc))))))
	      ((= len 4) (if (key-eq (nth 0 desc) '|function|)
                         (progn
                           (setq type (nth 0 desc))
		                   (setq modifier '|*|)
                           (setq variable (nth 1 desc))
				           (setq array (specify-function desc (list '(|declare|)) 0 ir t)))
                         (if (key-eq (nth 0 desc) '|const|)
		                     ...
		                     (if (key-eq (nth 2 desc) '|const|)
			                     ...
			                     (progn
			                       ...
			                       (setq variable (nth 2 desc))
			                       (setq array (nth 3 desc)))))))
	      ((= len 5) (if (and (key-eq (nth 0 desc) '|const|) (key-eq (nth 1 desc) '|function|))
                         (progn
                           (setq const (nth 0 desc))
                           (setq type (nth 1 desc))
		                   (setq modifier '|*|)
		                   (setq variable (nth 2 desc))
				           (setq array (specify-function (cdr desc) (List '(|declare|)) 0 ir t)))
    ...
    (unless (or (null array) (is-array array) (key-eq type '|function|)) (setq status -7))
             ...
             (if (key-eq (car quoted) '|lambda|)
                 (progn ; lambda
                   (let ((ldesc (cdr (nthcdr (- (length desc) 2) desc)))
                     ...
                     (display "QQLL" (caar ldesc) (append (list '|function| lname) (cdar ldesc)))
                     (when (< *ast-run* 3)                    
                       (specify-function (append (list '|function| lname) (cdar ldesc)) '() 0 (inners *function-spec*)))
                     (multiple-value-bind (const type modifier const-ptr variable array)
		                 (specify-type< (without-last wl) ir)
		               (values const type modifier const-ptr variable array lname))))
                 (progn ; list
...
(defun specify-function (def attrs lvl ir &optional (asType nil))
  (let* ((is-static nil)
	     (is-declare nil)
	     (is-inline nil)
	     (is-extern nil)
