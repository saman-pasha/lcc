...
(defun compile-target (spec globals stdout stderr dump header)
  (let ((file (format nil "~A" (name spec)))
	    (args (attrs spec)))
    (if (stringp file)
        (progn
          (setf *target-file* (file-namestring file))
	         ...
	         (progn
	           ...
	           (maphash #'(lambda (in-name in-spec)
		                    (case (construct in-spec)
			                  ...
			                  ('|@FUNCTION| (compile-lambda       in-spec 0 globals))
			                  ('|@METHOD|   (compile-lambda       in-spec 0 globals))
