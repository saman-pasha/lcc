...
(defun format-type-value (const typeof modifier const-ptr name array-def default anonymous globals)
  (when anonymous (setq name (format nil "/* ~A */" name)))
  (format-type const typeof modifier const-ptr name array-def anonymous globals)
...
(defun compile-spec-type-value (spec globals)
  (let ((const     (const     spec))
	    (typeof    (typeof    spec))
	    (modifier  (modifier  spec))
	    (const-ptr (const-ptr spec))
    ...
    (if (key-eq typeof '|func|)
        ...
        (format-type-value const typeof modifier const-ptr name array-def default anonymous globals))))
